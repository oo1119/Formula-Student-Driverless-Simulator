...
class PIDController:
    # ... (이전과 동일)
    def __init__(self, gains, initial_target): self.gains, self.target, self.integral, self.prev_error, self.prev_value, self.last_time = gains, initial_target, 0, 0, 0.0, rospy.Time.now()
...
class FinalController:
    ...
    def gps_callback(self, msg: NavSatFix): # ... (이전과 동일)
    ...
    def lidar_callback(self, msg: PointCloud2): # ... (이전과 동일)
    ...
    def control_logic(self): # ... (이전과 동일)
        if self.startup_mode_active:
            if self.current_speed > CREEP_SPEED_THRESHOLD: self.startup_mode_active = False; rospy.loginfo("Startup complete.")
            else: self.publish_control(CREEP_THROTTLE, 0.0); self.update_plot_data(self.detected_cones_local, [], [], None, 0.0); return
        ...
        if len(path_cones) >= 2:
            ...
            else: blue_path_cones = np.array([c for c in path_cones if c[1] > 0]); yellow_path_cones = np.array([c for c in path_cones if c[1] < 0])
        ...
        if len(blue_path_cones) >= 1 and len(yellow_path_cones) >= 1:
            ...
            if len(blue_path_cones) < len(yellow_path_cones):
                for b_cone in blue_path_cones:
                    distances = np.linalg.norm(yellow_path_cones - b_cone, axis=1); y_cone = yellow_path_cones[np.argmin(distances)]; current_center_points.append((b_cone + y_cone) / 2.0)
            else:
                for y_cone in yellow_path_cones:
                    distances = np.linalg.norm(blue_path_cones - y_cone, axis=1); b_cone = blue_path_cones[np.argmin(distances)]; current_center_points.append((b_cone + y_cone) / 2.0)
            self.path_history.extend(current_center_points)
        if len(self.path_history) >= 5:
            path_history_np = np.array(self.path_history)
            if len(path_history_np) >= 6:
                center_coeffs_2d = np.polyfit(path_history_np[:, 0], path_history_np[:, 1], 2)
            ...
            center_coeffs_1d = np.polyfit(path_history_np[:, 0], path_history_np[:, 1], 1)
            x_points = np.arange(0.5, LIDAR_RANGE_CUTOFF, 0.5); y_points = np.polyval(center_coeffs_1d, x_points)
        ...
        if not path_found: rospy.logwarn_throttle(1.0, "Path not found, holding last steering!"); throttle = 0.0
    ...
    def run(self): # ... (이전과 동일)
        ...
        while not rospy.is_shutdown(): self.control_logic(); plt.draw(); plt.pause(0.001); rate.sleep()
    def setup_plot(self): # ... (이전과 동일)
        self.ax.set_title("Path History Pure Pursuit Controller"); self.ax.set_xlabel("X (local, m)"); self.ax.set_ylabel("Y (local, m)")
        ...
        self.center_path_plot, = self.ax.plot([], [], 'g--', linewidth=2, label="Center Path from History")
        self.car_plot, = self.ax.plot([], [], 'r>', markersize=15, label="Car")
